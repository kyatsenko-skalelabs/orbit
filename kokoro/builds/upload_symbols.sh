...
function set_api_key {
  local header="$(./oauth2l header cloud-platform userinfo.email)"
  ...
  export CRASH_SYMBOL_COLLECTOR_API_KEY=$(eval $gcurl https://apikeys.googleapis.com/v2beta1/$crash_symbol_key_full_name/keyString | jq '.keyString' | tr -d '"')
}
...
function upload_symbol_file {
  ...
  if [ "$(uname -s)" == "Linux" ]; then
    ...
    local sym_file_path="$symbol_file_path"
  fi
